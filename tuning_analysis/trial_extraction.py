...
def prepare_DIO_data(session_restriction : dict, 
                    lick_event_threshold: float = 2,
                    dio1 = "center poke",
                    dio2 = "left poke",
                    dio3 = "right poke"):
    """ 
...
# ---------------------------------------------- Main
    ...
    middle_dio = pd.DataFrame((sgc.DIOEvents() & session_restriction & {"dio_event_name": dio1}).fetch_nwb()[0]["dio"].data).rename(columns = {0:"middle"})
    middle_timestamps = pd.DataFrame((sgc.DIOEvents() & session_restriction & {"dio_event_name": dio1}).fetch_nwb()[0]["dio"].timestamps).rename(columns = {0: "timestamps"})
    
    left_dio = pd.DataFrame((sgc.DIOEvents() & session_restriction & {"dio_event_name": dio2}).fetch_nwb()[0]["dio"].data).rename(columns = {0:"left"})
    left_timestamps = pd.DataFrame((sgc.DIOEvents() & session_restriction & {"dio_event_name": dio2}).fetch_nwb()[0]["dio"].timestamps).rename(columns = {0: "timestamps"})  
    
    right_dio = pd.DataFrame((sgc.DIOEvents() & session_restriction & {"dio_event_name": dio3}).fetch_nwb()[0]["dio"].data).rename(columns = {0:"right"})
    right_timestamps = pd.DataFrame((sgc.DIOEvents() & session_restriction & {"dio_event_name": dio3}).fetch_nwb()[0]["dio"].timestamps).rename(columns = {0: "timestamps"})
...
def merge_trial_df_with_target(target_dataframe, trials_dataframe, reward_ranges = (
    ...
    merged_dataframe["speed_norm"] = merged_dataframe["speed"]/merged_dataframe["speed"].max()
    ...
    merged_dataframe["trial_progress_distance"] = np.nan
    required_cols_mask = (
        merged_dataframe["trial_number"].notna()
        & merged_dataframe["linear_position"].notna()
    )
    trialized_position_for_progress = merged_dataframe.loc[
        required_cols_mask, ["trial_number", "linear_position"]
    ].copy()

    for trial_number, trial_df in trialized_position_for_progress.groupby("trial_number", sort=False):
        trial_df_sorted = trial_df.sort_index()

        delta_linear_distance = trial_df_sorted["linear_position"].astype(float).diff().abs().fillna(0.0)
        cumulative_linear_distance = delta_linear_distance.cumsum()

        total_linear_distance = float(cumulative_linear_distance.max())
        if total_linear_distance > 0:
            trial_progress_distance = (cumulative_linear_distance / total_linear_distance).clip(0.0, 1.0)
        else:
            trial_progress_distance = np.full(len(trial_df_sorted), np.nan)

        merged_dataframe.loc[
            trial_df_sorted.index, "trial_progress_distance"
        ] = np.asarray(trial_progress_distance, dtype=float)
